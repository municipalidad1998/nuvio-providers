...
class LatAnimeScraper extends StreamScraper {
  @override
  String get name => 'PlayTorrioHTTP';

  static const _baseUrl = 'https://latanime.org';
  ...
  Stream<StreamSource> scrapeStream({
    ...
    try {
      ...
      if (kDebugMode) debugPrint('[LatAnimeScraper] error: $e');
    }
  }
  ...
  Stream<StreamSource> _extractStreams(String pageUrl) async* {
    ...
    final direct = extractMediaUrls(html);
    for (final url in direct) {
      if (seen.add(url)) {
        yield _makeSource(url, 'Servidor principal', pageUrl);
      }
    }
    final candidates = <MapEntry<String, String>>[];
    for (final u in extractIframeUrls(html)) {
      candidates.add(MapEntry(u, 'Servidor'));
    }
    for (final u in extractBase64EmbedUrls(html)) {
      candidates.add(MapEntry(u, 'Servidor'));
    }
    for (final v in extractVideosArrayEmbeds(html)) {
      candidates.add(MapEntry(v.value, v.key));
    }
    var serverIdx = 1;
  ...
  StreamSource _makeSource(String url, String label, String sourceUrl) {
    final q = RegExp(r'(\d{3,4})p').firstMatch(url);
    return StreamSource(
      name: 'PlayTorrioHTTP',
      addonName: 'PlayTorrioHTTP',
      title: 'LatAnime · $label',
      url: url,
      headers: {
