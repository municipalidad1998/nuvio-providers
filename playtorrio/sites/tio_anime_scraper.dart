...
class TioAnimeScraper extends StreamScraper {
  @override
  String get name => 'PlayTorrioHTTP';

  static const _baseUrl = 'https://tioanime.com';
  static const _episodePath = '/ver/{slug}-{episode}';
  ...
  Stream<StreamSource> scrapeStream({
    ...
    try {
      ...
      final detailUrl = await searchSiteGeneric(
        baseUrl: _baseUrl,
        searchPattern: '/directorio?q=',
        title: mediaTitle,
      );
      var pageUrl = detailUrl;
      if (isTv && episode != null) {
        final slug = detailUrl.split('?')[0].split('#')[0].replaceAll(RegExp(r'/+$'), '').split('/').last;
        final path = _episodePath
            .replaceAll('{slug}', slug)
            .replaceAll('{season}', '${season ?? 1}')
            .replaceAll('{episode}', '$episode');
        final candidate = path.startsWith('http') ? path : '$_baseUrl$path';
        try {
          await getHtml(candidate);
          pageUrl = candidate;
        } catch (_) {
      ...
      if (kDebugMode) debugPrint('[TioAnimeScraper] error: $e');
    }
  }
