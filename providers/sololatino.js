...
function log() {
  const args = ['[SoloLatino]'].concat(Array.prototype.slice.call(arguments));
...
function getStreams(tmdbId, mediaType, season, episode) {
  ...
  return request('https://api.themoviedb.org/3/' + endpoint + '/' + tmdbId + '?api_key=' + TMDB_API_KEY + '&language=es-ES', { json: true })
    ...
    .then(function(searchData) {
      ...
      const typeSlug = best.type || (isTv ? 'serie' : 'pelicula');
      ...
      return request(detailUrl).then(function(detailHtml) {
        const tokenMatch = detailHtml.match(/token["']\s*:\s*["']([^"']+)/);
        const token = tokenMatch ? tokenMatch[1] : null;

        const serverRe = /\[\s*["']([^"']{1,40})["']\s*,\s*["'](https?:\/\/[^"']+)["']/g;
        const servers = [];
        let sm;
        const cleanHtml = detailHtml.replace(/\\\//g, '/');
        while ((sm = serverRe.exec(cleanHtml)) !== null) {
          servers.push({ name: sm[1], url: sm[2] });
        }
        log('Found ' + servers.length + ' server(s)');

        const promises = servers.map(function(svr) {
          const postHeaders = {
            'User-Agent': UA,
          ...
          return fetch(BASE_URL + '/api/player-url', {
            ...
            const streamUrl = (playerData && playerData.url) || (playerData && playerData.data && playerData.data.url);
            if (!streamUrl) return [];
            return [{
              name: 'SoloLatino',
              title: svr.name,
